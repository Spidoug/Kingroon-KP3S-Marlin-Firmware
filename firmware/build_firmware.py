...
def patch_configuration():
    ...
    set_define(cfg, "STRING_CONFIG_H_AUTHOR", '#define STRING_CONFIG_H_AUTHOR "by @SPIDOUG" // KP3S Marlin Firmware V1', "Set V1 firmware author")
...
''',
  ...
  enum class KP3SResonanceUIState : uint8_t { NONE, RUNNING, APPLIED, LOW_CONF, HOME_NEEDED, Z_TOO_LOW, BUSY, NO_MPU, NO_SPACE, CAPTURE_FAIL, SAVE_FAIL };
  ...
  static void screen_kp3s_resonance_result() {
    ...
    else { strcpy_P(freq, PSTR("FREQ: --.-")); strcpy_P(conf, PSTR("CONF: --")); }
    ...
    switch (kp3s_res_state) {
      ...
      case KP3SResonanceUIState::HOME_NEEDED: status = kp3s_tr(F("HOME XYZ FIRST"),F("FACA HOME XYZ"),F("HOME XYZ PRIM."),F("HOME XYZ REQUIS"),F("XYZ HOMEN")); break;
      case KP3SResonanceUIState::Z_TOO_LOW: status = kp3s_tr(F("RAISE Z >= 5MM"),F("SUBA Z >= 5MM"),F("SUBA Z >= 5MM"),F("MONTER Z >=5MM"),F("Z >=5MM HEBEN")); break;
      case KP3SResonanceUIState::BUSY: status = kp3s_tr(F("PRINTER BUSY"),F("IMPRESSORA OCUP."),F("IMPRESORA OCUP."),F("IMPR. OCCUPEE"),F("DRUCKER BELEGT")); break;
  ...
  static void kp3s_run_resonance_axis(const AxisEnum axis) {
    kp3s_res_axis = axis; kp3s_res_frequency_hz = 0.0f; kp3s_res_confidence = 0;
    // ACTION_ITEM doesn't push a screen automatically. Preserve the resonance menu
    // so BACK always returns one level, including early validation failures.
    ui.push_current_screen();
    if (printingIsActive() || printingIsPaused() || planner.has_blocks_queued()) {
    ...
    if (axis_should_home(axis) || axis_should_home(Z_AXIS)) {
      kp3s_res_state=KP3SResonanceUIState::HOME_NEEDED;
      ui.goto_screen(screen_kp3s_resonance_result); return;
    }
    // Never launch a horizontal resonance impulse with the nozzle on/near the
    // bed. The user keeps full control of Z instead of the tuner moving it.
    if (current_position.z < 5.0f) {
      kp3s_res_state=KP3SResonanceUIState::Z_TOO_LOW;
      ui.goto_screen(screen_kp3s_resonance_result); return;
    }
    ...
    const float original = current_position[axis];
    const float center = _MAX(amin + 5.0f, _MIN(amax - 5.0f, original));
    ...
    ui.refresh(LCDVIEW_CALL_REDRAW_NEXT);
    safe_delay(80); // Give the Nokia one redraw before motion begins.
    ...
    if (!kp3s_mpu6050_resonance_capturing()) {
      ...
      if (axis == X_AXIS) do_blocking_move_to_x(original, 45.0f); else do_blocking_move_to_y(original, 45.0f);
      planner.synchronize();
      kp3s_res_state=KP3SResonanceUIState::CAPTURE_FAIL;
      ui.refresh(LCDVIEW_CALL_REDRAW_NEXT); return;
    }
    if (axis == X_AXIS) { do_blocking_move_to_x(hi, 120.0f); do_blocking_move_to_x(lo, 120.0f); }
    else                { do_blocking_move_to_y(hi, 120.0f); do_blocking_move_to_y(lo, 120.0f); }
    safe_delay(850);
...
static void screen_kp3s_about() {
  ...
  MenuItem_static::draw(2, kp3s_tr(F("by @SPIDOUG"), F("por @SPIDOUG"), F("por @SPIDOUG"), F("par @SPIDOUG"), F("von @SPIDOUG")), SS_CENTER | SS_INVERT);
...
def verify_project():
    ...
    files_and_markers = {
        OUT / "Marlin" / "Configuration.h": [
            ...
            '#define STRING_CONFIG_H_AUTHOR "by @SPIDOUG"',
        ...
        OUT / "Marlin" / "src" / "lcd" / "menu" / "menu_configuration.cpp": [
            ...
            "stepper.set_shaping_frequency(axis, kp3s_res_frequency_hz)",
            "axis_should_home(axis) || axis_should_home(Z_AXIS)",
