...
def safe_sd_name(source: Path, requested: str | None) -> str:
    ...
    # FAT keeps DOS device names reserved even when an extension is present.
...
def require_pyserial() -> None:
    if serial is None or list_ports is None:
        raise TransferError("pyserial is required. Run SERIAL_SPOOL.bat or: python -m pip install pyserial")
...
def probe_capabilities(session: BinarySession) -> None:
    ...
    identity = " ".join(lines)
    if "MACHINE_TYPE:KINGROON KP3S V1" not in identity:
        raise TransferError("Connected printer is not identified as KINGROON KP3S V1. Refusing upload.")
...
def main() -> int:
    ...
    if source.stat().st_size == 0:
        raise TransferError("Input G-code file is empty.")
    ...
    with serial.Serial(port, baudrate=args.baud, timeout=0.20, write_timeout=5.0) as ser:
        ...
        try:
            ...
            if file_open:
                session.abort_file()
            # Best-effort protocol close on an already-failed transfer. Preserve the
            # original transfer error because no print will be started afterwards.
            try:
        ...
        # A successful upload is not enough: ASCII commands are unsafe until the
        # binary stream has positively returned to ASCII mode. Never start M23/M24
        # if this transition fails.
        session.leave_binary()
